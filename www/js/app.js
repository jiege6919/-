...
(function($){ 
    ...
    var oFReader = new FileReader(),
        rFilter = /^(?:image\/gif|image\/ief|image\/jpeg|image\/jpeg|image\/jpeg|image\/pipeg|image\/png)$/i;
    ...
    if (backgroundImage) {
    $('.header-big').css('background-image', 'url(' + backgroundImage + ')');    
    } 
    else {
    $('.header-big').css('background-image', 'url(https://zxso.net/api/bing)');
    }
    }
// 大熊猫的壁纸地址:https://xn--wcsw84d.rth.app/xiongmao.jpg
    ...
    $(document).on('click', "a[target!='_blank']", function() {
        if( theme.loading=='1' && $(this).attr('href') && $(this).attr('href').indexOf("#") != 0 && $(this).attr('href').indexOf("java") != 0 && !$(this).data('fancybox')  && !$(this).data('commentid') ){
    ...
    $(".add-custom-site-form").on("submit", function() {
        ...
        .done(function(response) {   
            ...
            '<div class="url-body mini"><a href="'+url+'" target="_blank" class="card new-site mb-3 site-'+id+'" data-id="'+id+'" data-url="'+url+'" data-toggle="tooltip" data-placement="bottom" title="'+name+'" rel="external nofollow">'+
                ...
                '</div>'+
            '</a></div>' +
            '<a href="javascript:;" class="text-center remove-site" data-action="delete_custom_url" data-id="'+id+'"><i class="iconfont icon-close-circle"></i></a>'+
            '</div>');
    ...
    $(document).on("click",'.url-card .remove-site', function(event){ 
        var t = $(this); 
        $.ajax({
            url: theme.ajaxurl,
            type: 'POST', 
